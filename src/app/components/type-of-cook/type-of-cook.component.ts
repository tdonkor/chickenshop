...
@Component({
  selector: 'acr-type-of-cook',
  templateUrl: './type-of-cook.component.html',
  styleUrls: ['./type-of-cook.component.scss'],
  animations: [Animations.popupIn, Animations.popupOut],
})
export class TypeOfCookComponent
  ...
  public selectCookType(linkType) {

    this.modifiers.map(val => {
      val.Buttons.map(btn => {
        btn.Selected = btn.Link === linkType ? true : false
      })
    })
    this.cancelClick()
